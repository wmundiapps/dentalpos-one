import type { ImplantRecord, ProsthesisRecord, SurgeryRecord, LaboratoryPendingState, LaboratoryResolutionInput } from '../types/specialtyClinical'
...
export const SpecialtyClinicalApi = {
  ...
  laboratoryPendingState: (id:string, kind='LABORATORY_OVERDUE') =>
    request<LaboratoryPendingState>(`/laboratory-works/${id}/pending-state?kind=${encodeURIComponent(kind)}`),

  beginLaboratoryPendingTreatment: (id:string, input:Record<string,unknown>) =>
    request<LaboratoryPendingState>(`/laboratory-works/${id}/pending-treatment`, {
      method:'POST',
      body:JSON.stringify(input)
    }),

  resolveLaboratoryPending: (id:string, input:LaboratoryResolutionInput) =>
    request<LaboratoryPendingState>(`/laboratory-works/${id}/resolve-pending`, {
      method:'POST',
      body:JSON.stringify(input)
    }),
}
