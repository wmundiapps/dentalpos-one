...
export default function Sidebar(){
  ...
  const initials=`${sessionUser?.firstName?.[0]||""}${sessionUser?.lastName?.[0]||""}`.toUpperCase()||"EM";
  ...
  return <Box component="aside" sx={{
    ...
    {demo?.isDemo ? (
      collapsed ? (
        <Tooltip title={`${daysRemaining} dia${daysRemaining===1?"":"s"} restante${daysRemaining===1?"":"s"} — Assinar EduMaster Pro`} placement="right">
          <IconButton
        ...
        <Box sx={{mx:1,mb:1.5,p:1.5,borderRadius:2,bgcolor:"#1E293B",border:"1px solid #334155"}}>
          ...
          >
            Assinar EduMaster Pro
          </Button>
        </Box>
      )
    ) : (
      <Box sx={{py:1.2,textAlign:"center",color:"#64748B",whiteSpace:"nowrap"}}>
        {collapsed?<Typography sx={{fontSize:9,fontWeight:800}}>EM</Typography>:
    ...
    <Dialog open={showInDevelopment} onClose={()=>setShowInDevelopment(false)} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        <Typography>Esta funcionalidade está em desenvolvimento e será disponibilizada em breve no EduMaster Pro.</Typography>
      </DialogContent>
